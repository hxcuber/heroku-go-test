...
func (i impl) CommonFriends(ctx context.Context, email1 string, email2 string) ([]string, error) {
	...
	err := i.repo.DoInTx(context.Background(), func(ctx context.Context, txRepo repository.Registry) error {
		...
		if err != nil {
			log.Printf(LogErrMessage("CommonFriends", "retrieving user1 by email %s", err, email1))
			return err
		}

		user2, err := txRepo.Relationship().GetUserByEmail(ctx, email2)
		if err != nil {
			log.Printf(LogErrMessage("CommonFriends", "retrieving user2 by email %s", err, email2))
			return err
		}

		user1Friends, err = txRepo.Relationship().GetFriends(ctx, user1)
		if err != nil {
			log.Printf(LogErrMessage("CommonFriends", "retrieving user1 friends", err))
			return err
		}
		user2Friends, err = txRepo.Relationship().GetFriends(ctx, user2)
		if err != nil {
			log.Printf(LogErrMessage("CommonFriends", "retrieving user2 friends", err))
			return err
		}
	...
	if err != nil {
		log.Printf(LogErrMessage("CommonFriends", "doing in transaction", err))
		return nil, err
	}
