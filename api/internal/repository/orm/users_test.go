...
func testUserToManyReceiverRelationships(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	b.ReceiverID = a.UserID
	c.ReceiverID = a.UserID
	...
	check, err := a.ReceiverRelationships().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadReceiverRelationships(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ReceiverRelationships); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ReceiverRelationships = nil
	if err = a.L.LoadReceiverRelationships(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ReceiverRelationships); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManySenderRelationships(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.SenderRelationships().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadSenderRelationships(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SenderRelationships); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.SenderRelationships = nil
	if err = a.L.LoadSenderRelationships(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SenderRelationships); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpReceiverRelationships(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddReceiverRelationships(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.UserID != first.ReceiverID {
			t.Error("foreign key was wrong value", a.UserID, first.ReceiverID)
		}
		if a.UserID != second.ReceiverID {
			t.Error("foreign key was wrong value", a.UserID, second.ReceiverID)
		}
		...
		if a.R.ReceiverRelationships[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ReceiverRelationships[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ReceiverRelationships().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpSenderRelationships(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddSenderRelationships(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.UserID != first.SenderID {
			t.Error("foreign key was wrong value", a.UserID, first.SenderID)
		}
		if a.UserID != second.SenderID {
			t.Error("foreign key was wrong value", a.UserID, second.SenderID)
		}
		...
		if a.R.SenderRelationships[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.SenderRelationships[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.SenderRelationships().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`UserID`: `bigint`, `UserEmail`: `text`}
	_           = bytes.MinRead
)
