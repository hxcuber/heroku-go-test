...
func (i impl) Befriend(ctx context.Context, email1 string, email2 string) error {
	err := i.repo.DoInTx(context.Background(), func(ctx context.Context, txRepo repository.Registry) error {
		...
		if err != nil {
			log.Printf(LogErrMessage("Befriend", "retrieving user1 by email %s", err, email1))
			return err
		}

		user2, err := txRepo.Relationship().GetUserByEmail(ctx, email2)
		if err != nil {
			log.Printf(LogErrMessage("Befriend", "retrieving user2 by email %s", err, email2))
			return err
		}
		...
		if err != nil {
			if !errors.Is(err, sql.ErrNoRows) {
				log.Printf(LogErrMessage("Befriend", "retrieving relationship 1 to 2", err))
				return err
			}
			...
			if err != nil {
				log.Printf(LogErrMessage("Befriend", "creating relationship 1 to 2", err))
				return err
			}
		} else {
			...
			case orm.StatusRBlockedS:
				{
					log.Printf(LogErrMessage("Befriend", "controller logic", ErrBlocked))
					return ErrBlocked
				}
			case orm.StatusFriends:
				{
					log.Printf(LogErrMessage("Befriend", "controller logic", ErrAlreadyCreated))
			...
			if err != nil {
				log.Printf(LogErrMessage("Befriend", "updating relationship 1 to 2", err))
				return err
			}
		}
		...
		if err != nil {
			if !errors.Is(err, sql.ErrNoRows) {
				log.Printf(LogErrMessage("Befriend", "finding relationship 2 to 1", err))
				return err
			}
			...
			if err != nil {
				log.Printf(LogErrMessage("Befriend", "creating relationship 2 to 1", err))
				return err
			}
		} else {
			...
			if err != nil {
				log.Printf(LogErrMessage("Befriend", "updating relationship 2 to 1", err))
				return err
			}
		}
	...
	if err != nil {
		log.Printf(LogErrMessage("Befriend", "doing in transaction", err))
		return err
	}
	return nil
}
