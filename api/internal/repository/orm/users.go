...
var UserRels = struct {
	ReceiverRelationships string
	SenderRelationships   string
}{
	ReceiverRelationships: "ReceiverRelationships",
	SenderRelationships:   "SenderRelationships",
}

// userR is where relationships are stored.
type userR struct {
	ReceiverRelationships RelationshipSlice `boil:"ReceiverRelationships" json:"ReceiverRelationships" toml:"ReceiverRelationships" yaml:"ReceiverRelationships"`
	SenderRelationships   RelationshipSlice `boil:"SenderRelationships" json:"SenderRelationships" toml:"SenderRelationships" yaml:"SenderRelationships"`
}
...
func (r *userR) GetReceiverRelationships() RelationshipSlice {
	if r == nil {
		return nil
	}
	return r.ReceiverRelationships
}

func (r *userR) GetSenderRelationships() RelationshipSlice {
	if r == nil {
		return nil
	}
	return r.SenderRelationships
}
...
var (
	userAllColumns            = []string{"user_id", "user_email"}
	userColumnsWithoutDefault = []string{"user_email"}
...
// ReceiverRelationships retrieves all the relationship's Relationships with an executor via receiver_id column.
func (o *User) ReceiverRelationships(mods ...qm.QueryMod) relationshipQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"relationships\".\"receiver_id\"=?", o.UserID),
	)

	return Relationships(queryMods...)
}

// SenderRelationships retrieves all the relationship's Relationships with an executor via sender_id column.
func (o *User) SenderRelationships(mods ...qm.QueryMod) relationshipQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"relationships\".\"sender_id\"=?", o.UserID),
	)

	return Relationships(queryMods...)
}

// LoadReceiverRelationships allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadReceiverRelationships(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`relationships.receiver_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.ReceiverRelationships = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UserID == foreign.ReceiverID {
				local.R.ReceiverRelationships = append(local.R.ReceiverRelationships, foreign)
				if foreign.R == nil {
					foreign.R = &relationshipR{}
				}
...
// LoadSenderRelationships allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadSenderRelationships(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`relationships`),
		qm.WhereIn(`relationships.sender_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.SenderRelationships = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &relationshipR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UserID == foreign.SenderID {
				local.R.SenderRelationships = append(local.R.SenderRelationships, foreign)
				if foreign.R == nil {
					foreign.R = &relationshipR{}
				}
...
// AddReceiverRelationships adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.ReceiverRelationships.
// Sets related.R.Receiver appropriately.
func (o *User) AddReceiverRelationships(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Relationship) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ReceiverID = o.UserID
	...
	if o.R == nil {
		o.R = &userR{
			ReceiverRelationships: related,
		}
	} else {
		o.R.ReceiverRelationships = append(o.R.ReceiverRelationships, related...)
	}
...
// AddSenderRelationships adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.SenderRelationships.
// Sets related.R.Sender appropriately.
func (o *User) AddSenderRelationships(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Relationship) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.SenderID = o.UserID
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
	...
	if o.R == nil {
		o.R = &userR{
			SenderRelationships: related,
		}
	} else {
		o.R.SenderRelationships = append(o.R.SenderRelationships, related...)
	}
